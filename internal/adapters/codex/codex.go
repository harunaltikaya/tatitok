...
// checks: rollout files reach hundreds of megabytes, and honoring
...
// checking ctx every cancelCheckInterval lines. The error returns are
// distinct on purpose: readErr means the FILE could not be (fully) read
// — Backfill reports it as a skipped source; sinkErr (also carrying ctx
// cancellation) must cancel the whole backfill unchanged.
