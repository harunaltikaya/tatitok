...
// transaction. INSERT OR IGNORE on the primary key makes re-ingest
// idempotent. Returns the number of newly inserted (non-duplicate) events.
func (s *Store) InsertBatch(ctx context.Context, events []core.Event, src SourceInfo) (int, error) {
	...
	stmt, err := tx.PrepareContext(ctx, `INSERT OR IGNORE INTO usage_events
		(id, ts, machine, source_kind, harness, provider, model, model_family,
		 ...
		 tokens_reasoning, accuracy, meta, raw)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
	if err != nil {
		return 0, err
	}
	...
	for i := range events {
		...
		if e.Meta != nil {
			...
			if err != nil {
				return 0, fmt.Errorf("marshal meta for %s: %w", e.ID, err)
			}
			meta = string(b)
		}
		var raw any
